...
import { ArrowRight, Play, BookOpen, Briefcase, FolderOpen } from "lucide-react";
import { AnimatedSphere } from "./animated-sphere";
import Link from "next/link";

const words = ["learn", "build", "showcase", "grow"];
    ...
    <section className="relative min-h-screen flex flex-col justify-center overflow-hidden">
      ...
      <div className="relative z-10 max-w-[1400px] mx-auto px-6 lg:px-12 py-32 lg:py-40">
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-24 items-end">
          ...
          >
            Your all-in-one platform for video learning, internships,
            and project showcasing. Start your journey today.
          </p>
        ...
        <div className="flex gap-16 marquee whitespace-nowrap">
          {[...Array(2)].map((_, i) => (
            <div key={i} className="flex gap-16">
              {[
                { value: "500+", label: "video lectures", icon: "📹" },
                { value: "1,200+", label: "projects showcased", icon: "🚀" },
                { value: "300+", label: "internees placed", icon: "💼" },
                { value: "50+", label: "skill tracks", icon: "📚" },
              ].map((stat) => (
                <div key={`${stat.label}-${i}`} className="flex items-baseline gap-4">
                  <span className="text-4xl lg:text-5xl font-display">{stat.value}</span>
                  <span className="text-sm text-muted-foreground">
                    {stat.label}
                    <span className="block font-mono text-xs mt-1">{stat.icon}</span>
                  </span>
                </div>
              ))}
            </div>
